      ...
      {filteredUnsoldPlayers.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {filteredUnsoldPlayers.map(player => (
            <div
              key={player.id}
              className={`bg-background rounded-lg overflow-hidden group relative border-2 transition-all ${
                selectedPlayerIds.includes(player.id) ? 'border-primary scale-105' : 'border-transparent'
              }`}
            >
              <div
              ...
              <button
                ...
                className="absolute top-2 right-2 text-xs bg-gray-800/70 text-white px-2 py-1 rounded-md opacity-0 group-hover:opacity-100 transition-opacity"
              >
                Details
              </button>
            </div>
