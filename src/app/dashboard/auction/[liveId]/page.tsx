...
const [popupColor, setPopupColor] = useState<string>('border-gray-400');
    ...
    const handleAdminBid = (teamId: string) => {
        const increment = parseInt(adminBidIncrements[teamId] || '0');
    ...
    //     if(!isAuctionActive && playerBidHistory.length > 0) return <p className="text-lg font-bold text-yellow-400">UNSOLD</p>;
    //     if(!isAuctionActive && !winningTeam) return <p className="text-lg font-bold text-gray-400">Ready to Start</p>;
    //     if(isAuctionActive && biddingTurnTeam) return <p className="text-lg font-bold text-yellow-400">{biddingTurnTeam.name}'s Turn</p>;
    //     if(isAuctionActive && !biddingTurnTeamId) return <p className="text-lg font-bold text-cyan-400">Selecting next bidder...</p>;
  ...
  if (winningTeam) {
    message = `SOLD to ${winningTeam.name}!`;
    color = 'border-green-400';
  } else if (!isAuctionActive && playerBidHistory.length > 0) {
    message = 'UNSOLD';
    color = 'border-yellow-400';
  } else if (!isAuctionActive && !winningTeam) {
    message = 'Ready to Start';
    color = 'border-gray-400';
  } else if (isAuctionActive && biddingTurnTeam) {
    message = `${biddingTurnTeam.name}'s Turn`;
    color = 'border-yellow-400';
  } else if (isAuctionActive && !biddingTurnTeamId) {
    message = 'Selecting next bidder...';
  ...
  if (message) {
    ...
    const timeout = setTimeout(() => setPopupMessage(null), 3000); // auto-close in 3s
    return () => clearTimeout(timeout);
  }
}, [winningTeam, isAuctionActive]);
                        ...
                        <div className="text-center md:text-left mb-4 md:mb-0">
                            ...
                            <p className="text-4xl font-bold text-primary">{(currentBid?.amount || currentPlayer.baseCoins).toLocaleString()} coins</p>
                             <p className="text-sm text-text-secondary">
                                {currentBid ? `by ${highestBidder?.name}` : 'Base Coins'}
                            </p>
                        </div>
  ...
  />
                    ...
                    {bidError && <p className="text-center text-red-500 font-semibold mt-2 bg-red-500/10 p-2 rounded-lg">{bidError}</p>}
                </div>
                    ...
                    <div className="bg-surface p-6 rounded-xl flex flex-col sm:flex-row items-center justify-between">
                        <div className="flex items-center space-x-4 mb-4 sm:mb-0">
                            <div className="flex items-center space-x-2">
                                 <label htmlFor="timer-set" className="text-text-secondary font-semibold">Timer (s):</label>
                                 <input 
                                    id="timer-set"
                                    type="number"
                            ...
                            <div className="flex items-center space-x-2">
                                <label htmlFor="timer-toggle" className="text-text-secondary font-semibold">Enable Timer:</label>
                                <button onClick={() => setTimerEnabled(!isTimerEnabled)} className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors ${isTimerEnabled ? 'bg-secondary' : 'bg-gray-600'}`}>
                                    <span className={`inline-block w-4 h-4 transform bg-white rounded-full transition-transform ${isTimerEnabled ? 'translate-x-6' : 'translate-x-1'}`}/>
                                </button>
                             </div>
                        </div>
                         <button onClick={startAuctionForPlayer} className="w-full sm:w-auto bg-secondary text-white font-bold p-4 rounded-lg text-xl hover:bg-emerald-600 transition-colors duration-200">
                                    ...
                                    <div className="flex items-center space-x-3">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center space-x-2 text-xs text-text-secondary">
                                                <span className="flex items-center"><MoneyIcon className="w-3 h-3 mr-1"/>{team.coins.toLocaleString()} coins</span>
                                            </div>
                                        </div>
                                    </div>
                                    <span className="text-sm font-bold bg-primary text-white rounded-full w-6 h-6 flex items-center justify-center">{team.players.length}</span>
                                </div>
                                 {loggedInAdmin && isAuctionActive && (
                                    <div className="flex items-center justify-end mt-2 space-x-1">
                                        <input
                                            ...
                                            onChange={(e) => setAdminBidIncrements(prev => ({ ...prev, [team.id]: e.target.value }))}
                                        />
                                        <button
                ...
                <div className="bg-surface p-6 rounded-xl shadow-lg">
                    ...
                    <ul className="space-y-2 max-h-64 overflow-y-auto pr-2">
                        {playerBidHistory.map((bid) => {
                            ...
                            return (
                                <li key={bid.id} className="flex justify-between items-center bg-background p-2 rounded-md text-sm">
                         ...
                         {playerBidHistory.length === 0 && <p className="text-text-secondary text-center">No bids yet.</p>}
                    </ul>
                </div>
            </div>
        </div>
