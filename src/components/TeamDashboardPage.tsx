...
const TeamDashboardPage: React.FC<TeamDashboardPageProps> = ({ onViewPlayer }) => {
    // FIX: Replaced useMockData with useFirebase from the context.
    const { 
        loggedInTeamId, teams,
        timer, isAuctionActive,
        unsoldPlayers, currentPlayerIndex, currentBid, biddingTurnTeamId,
        placeBid, passTurn, timerTick
    } = useFirebase();
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="bg-surface p-6 rounded-xl shadow-lg flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-6">
                <img src={loggedInTeam.logoURL} alt={`${loggedInTeam.name} logo`} className="w-24 h-24 rounded-full object-cover border-4 border-primary"/>
                <div>
            ...
            {isAuctionActive && currentPlayer ? (
                <div className={`bg-surface p-6 rounded-xl shadow-lg border-2 ${isMyTurn ? 'border-yellow-400 animate-pulse' : 'border-transparent'}`}>
                    <h2 className="text-2xl font-bold mb-4">Live Auction: <span className="text-primary">{currentPlayer.name}</span></h2>
                     <div className="flex flex-col md:flex-row items-center justify-between bg-background p-4 rounded-lg">
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <p className={`text-5xl font-mono font-bold ${timer < 10 && timer > 0 ? 'text-red-500' : 'text-white'}`}>{timer}</p>
                        </div>
                        <div className="mt-4 md:mt-0 flex items-center space-x-3">
                           ...
                           <div className="flex items-center">
                              <span className="bg-gray-800 border border-gray-600 rounded-l-lg px-3 py-3 font-semibold">+</span>
                              <input type="number" value={bidIncrement} onChange={(e) => setBidIncrement(parseInt(e.target.value))} disabled={!isMyTurn} step="10000" min="0" className="w-32 px-3 py-3 bg-gray-800 border-t border-b border-gray-600 disabled:opacity-50"/>
                              <button 
                    ...
                    {isMyTurn && <p className="text-center mt-4 text-yellow-400 font-semibold">It's your turn to bid! (First bid can be at base coins by entering 0)</p>}
                    {bidError && <p className="text-center mt-4 text-red-500 font-semibold bg-red-500/10 p-2 rounded-lg">{bidError}</p>}
                </div>
            ) : (
            ...
            <div className="bg-surface p-6 rounded-xl shadow-lg">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    ...
                    <div key={player.id} className="bg-background p-3 rounded-lg flex items-center space-x-4 cursor-pointer hover:bg-gray-800" onClick={() => onViewPlayer(player.id)}>
