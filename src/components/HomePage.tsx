...
import { Page, Player, Team, Bid } from '../../types';
...
const HomePage: React.FC<HomePageProps> = ({ setCurrentPage }) => {
  ...
  return (
    <div className="space-y-12">
      ...
      <section className="text-center bg-surface rounded-lg p-8 md:p-16" style={{backgroundImage: `url('https://picsum.photos/1200/400?grayscale&blur=2')`, backgroundSize: 'cover', backgroundPosition: 'center'}}>
        <div className="bg-black/60 p-8 rounded-lg">
          ...
          <p className="max-w-3xl mx-auto text-lg md:text-xl text-text-secondary mb-8">
            The ultimate platform for dynamic, real-time cricket player auctions. Manage teams, discover talent, and experience the thrill of the bid.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button 
              onClick={() => setCurrentPage('live-auction')}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {featuredPlayers.map(p => (
                        <div key={p.id} className="text-center">
                            <img src={p.profileImageURL} alt={p.name} className="w-24 h-24 rounded-full mx-auto object-cover border-2 border-surface hover:border-primary transition-colors"/>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                     {featuredTeams.map(t => (
                        <div key={t.id} className="text-center">
                            <img src={t.logoURL} alt={t.name} className="w-24 h-24 rounded-full mx-auto object-cover border-2 border-surface hover:border-primary transition-colors"/>
