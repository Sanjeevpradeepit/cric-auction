...
import { Team, Owner, Page } from '../../types';
...
const TeamManagementPage: React.FC<TeamManagementPageProps> = ({ onSelectTeam, setCurrentPage }) => {
  // FIX: Replaced useMockData with useFirebase and destructured deleteTeam.
  ...
  const handleDeleteTeam = (teamId: string) => {
    if (window.confirm('Are you sure you want to delete this team?')) {
      // FIX: Use deleteTeam function from context.
      deleteTeam(teamId);
    }
  };
    ...
    <div className="max-w-7xl mx-auto space-y-8">
      <div className="bg-surface p-6 rounded-xl shadow-lg">
        <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4 gap-4">
          ...
          {loggedInAdmin && (
            <button
              onClick={() => setCurrentPage('add-team')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {teams.map(team => (
            <div key={team.id} className="bg-background p-4 rounded-lg flex flex-col items-center text-center space-y-3">
              <img src={team.logoURL} alt={`${team.name} logo`} className="w-20 h-20 rounded-full object-cover border-2 border-primary" />
              ...
              <div className="text-sm text-text-secondary">
               ...
               <div className="flex space-x-2 pt-2">
                 ...
                 {loggedInAdmin && (
                   <>
                    <button onClick={() => handleOpenEditModal(team)} className="p-2 bg-gray-700 rounded-full hover:bg-primary transition-colors" title="Edit Team"><EditIcon className="w-4 h-4" /></button>
                    <button onClick={() => handleDeleteTeam(team.id)} className="p-2 bg-gray-700 rounded-full hover:bg-red-500 transition-colors" title="Delete Team"><TrashIcon className="w-4 h-4" /></button>
                   </>
                 )}
              </div>
            </div>
      ...
      <Modal isOpen={!!editingTeam} onClose={() => setEditingTeam(null)} title="Edit Team Details">
        <form onSubmit={handleUpdateTeam} className="space-y-4">
            ...
            <div className="flex justify-end space-x-3 pt-4 border-t border-gray-700">
              <button type="button" onClick={() => setEditingTeam(null)} className="px-4 py-2 rounded-lg bg-gray-600 hover:bg-gray-500 text-white font-semibold">Cancel</button>
