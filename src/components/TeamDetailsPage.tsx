...
const TeamDetailsPage: React.FC<TeamDetailsPageProps> = ({ teamId, onBack, onViewPlayer }) => {
  // FIX: Replaced useMockData with useFirebase from the context.
  ...
  const isAuthorized = loggedInAdmin || loggedInTeamId === teamId;

  const teamBids = useMemo(() => {
    return bids.filter(b => b.teamId === teamId).sort((a, b) => b.timestamp - a.timestamp);
  }, [bids, teamId]);
    ...
    <div className="space-y-8">
      <div>
        ...
        <div className="bg-surface p-6 rounded-xl shadow-lg flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-6 relative">
          {isAuthorized && (
            <button onClick={() => setIsEditModalOpen(true)} className="absolute top-4 right-4 p-2 bg-gray-700 rounded-full hover:bg-primary transition-colors" title="Edit Team">
              <EditIcon className="w-5 h-5"/>
            </button>
          )}
          <img src={team.logoURL} alt={`${team.name} logo`} className="w-24 h-24 rounded-full object-cover border-4 border-primary"/>
          <div>
            <h1 className="text-4xl font-bold text-text-primary">{team.name}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="space-y-8">
          <div className="bg-surface p-6 rounded-xl shadow-lg">
              ...
              <div className="space-y-4">
                  ...
                  <div>
                      <h3 className="text-lg font-semibold text-text-secondary">Contact</h3>
                      <p className="text-sm text-text-primary">{team.email}</p>
                  </div>
                   <div>
                      <p className="text-sm text-text-secondary">Initial Coins</p>
                      <p className="text-xl font-bold">{(team.coins + team.players.reduce((sum, p) => sum + p.baseCoins, 0)).toLocaleString()} coins</p>
                  </div>
                   <div>
                      <p className="text-sm text-text-secondary">Total Spent</p>
                      <p className="text-xl font-bold text-red-400">{(team.players.reduce((sum, p) => sum + p.baseCoins, 0)).toLocaleString()} coins</p>
                  </div>
              </div>
          </div>
          <div className="bg-surface p-6 rounded-xl shadow-lg">
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
              {teamBids.length > 0 ? teamBids.map(bid => {
                const player = allPlayers.find(p => p.id === bid.playerId);
                return (
                  <div key={bid.id} className="bg-background p-3 rounded-lg text-sm">
                    <div className="flex justify-between items-center">
                      <p className="font-bold truncate">{player?.name || 'Unknown Player'}</p>
                      <p className="font-mono text-secondary">{bid.amount.toLocaleString()} coins</p>
                    </div>
                    <p className="text-xs text-text-secondary text-right">{new Date(bid.timestamp).toLocaleTimeString()}</p>
                  </div>
                );
              }) : <p className="text-text-secondary">No bids placed by this team yet.</p>}
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-surface p-6 rounded-xl shadow-lg">
             <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-6 gap-4">
                <h1 className="text-3xl font-bold">Player Roster ({filteredPlayers.length})</h1>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                ...
                <p className="col-span-full text-center text-text-secondary py-8">This team has not acquired any players yet.</p>
                 ...
                 {filteredPlayers.length === 0 && team.players.length > 0 && (
                  <p className="col-span-full text-center text-text-secondary py-8">No players match the current filters.</p>
                )}
            </div>
        </div>
      </div>
       <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title="Edit Team Details">
        <form onSubmit={handleUpdateTeam} className="space-y-4">
            ...
            <input name="name" value={editedTeamData.name || ''} onChange={handleEditInputChange} placeholder="Team Name" className="w-full px-3 py-2 bg-background border border-gray-600 rounded-lg" required />
            <input name="logoURL" value={editedTeamData.logoURL || ''} onChange={handleEditInputChange} placeholder="Logo URL" className="w-full px-3 py-2 bg-background border border-gray-600 rounded-lg" />
            
            {loggedInAdmin && (
              <>
                <div>
                    <label className="text-sm text-text-secondary">Coins: {editedTeamData.coins?.toLocaleString()} coins</label>
                    <input name="coins" type="range" min="1000000" max="20000000" step="500000" value={editedTeamData.coins || 0} onChange={handleEditInputChange} className="w-full" />
                </div>
                <input name="email" type="email" value={editedTeamData.email || ''} onChange={handleEditInputChange} placeholder="Login Email" className="w-full px-3 py-2 bg-background border border-gray-600 rounded-lg" required />
                <input name="password" type="text" value={editedTeamData.password || ''} onChange={handleEditInputChange} placeholder="New Password (optional)" className="w-full px-3 py-2 bg-background border border-gray-600 rounded-lg" />
              </>
            )}
            ...
            <div className="flex justify-end space-x-3 pt-4 border-t border-gray-700">
              <button type="button" onClick={() => setIsEditModalOpen(false)} className="px-4 py-2 rounded-lg bg-gray-600 hover:bg-gray-500 text-white font-semibold">Cancel</button>
