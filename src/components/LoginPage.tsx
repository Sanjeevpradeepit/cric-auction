...
const LoginPage: React.FC<LoginPageProps> = ({ setCurrentPage }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Auth state change will be handled by the context provider, redirecting the app.
  };
      ...
      <div className="max-w-md w-full bg-surface p-8 rounded-xl shadow-lg">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
         ...
         <p className="text-center text-text-secondary text-xs mt-4">
            ...
            <button onClick={() => setCurrentPage('home')} className="text-primary hover:underline">Home</button>
        </p>
      </div>
    </div>
  );
